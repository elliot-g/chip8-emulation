...
int main(void)
{
	...
	file_name_len = strlen(in_line);
	if (file_name_len > 0)
	{
		trim = 0;
		for (size_t idx = file_name_len - 1; idx > 0; idx--)
		{
			if (isspace(in_line[idx]))
				trim++;
			else
				break;
		}
		in_line[file_name_len - trim] = '\0'; // trim off excess garbage
	}
	...
	sdl_window = SDL_CreateWindow("Test Window for Chip 8",
	...
	{
		fprintf(stderr, "Are we out of memory yet?\n");
		exit(EXIT_FAILURE);
	}
